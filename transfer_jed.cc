/*
 ...
 transfer_jed.cc - This plugin supports Jed's Tk.
...
class transfer_jed_plugin : public transfer_function_plugin {
  ...
  void read_table(void) {
    ...
    if (MPI::COMM_WORLD.Get_rank() == 0) {
      ...
      while (!ifs.eof()) {
        ...
        ss >> dummy; // total_de
        ss >> dummy; // Weyl
        ss >> dummy;  //>[150609SH: add] // v_cdm
        ss >> dummy;  //>[150609SH: add] // v_b
        ss >> dummy; // v_b-v_cdm
...
public:
  transfer_jed_plugin(config_file &cf)
  : transfer_function_plugin(cf)
  {
    ...
    tf_distinct_ = false; // [150612SH: different density between CDM v.s. Baryon]
    tf_withvel_  = false; // [150612SH: using velocity transfer function]
  }
...
namespace {
transfer_function_plugin_creator_concrete<transfer_jed_plugin>
    creator("jed_file");
}
